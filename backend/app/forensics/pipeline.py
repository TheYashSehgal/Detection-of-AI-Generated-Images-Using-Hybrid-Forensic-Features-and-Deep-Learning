...
def get_forensic_summary(scores: dict) -> dict:
    ...
    Args:
        scores: dict with ela_score, srm_score, dct_score, and optionally ai_probability
    ...
    # Use the hybrid AI probability (CLIP + forensics) for the verdict
    # This ensures the forensic summary aligns with the main prediction
    ai_prob = scores.get('ai_probability', composite_score)
    
    # Interpretation based on overall AI probability
    if ai_prob > 0.7:
        ...
        forensic_verdict = "Low indicators of AI generation — likely authentic"
        risk_level = "low"
    
    return {
        'composite_score': ai_prob,
